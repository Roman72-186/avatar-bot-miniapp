# Real Estate AI Mini App — Turnkey Deployment

AI-сервис для риэлторов в Telegram Mini App: AI-ремонт, улучшение фото объекта, видео, текст объявления и Telegram Stars. Т-Банк пока отключен из рабочего процесса.
...
│   └── 04_real_estate_payments.sql # payments_log, generations.metadata
├── workflows/
...
### Режимы генерации недвижимости

| Режим | Стоимость | Описание |
|---|---|---|
| AI-ремонт | 20 кредитов | Визуализация ремонта и мебели |
| Улучшить фото | 8 кредитов | Свет, резкость, перспектива без изменения объекта |
| Видео объекта | 155 кредитов | Вертикальный ролик через рабочий video workflow |
| Текст объявления | 0 кредитов | Локальный черновик для Авито/Циан/Домклик/Telegram |
| Полный пакет | 155 кредитов | Видео + текст объявления |

Старые avatar/art режимы сохранены в коде как legacy, но скрыты из основного UI.

### Бесплатные генерации

- Для MVP недвижимости бесплатные генерации не включены.
- Старые дневные лимиты legacy-режимов сохранены в БД, но не показываются в основном UI.
...
## n8n для рабочего MVP

В текущем рабочем процессе frontend использует существующие активные endpoints:

- `create-invoice` для Telegram Stars;
- `generate` для AI-ремонта с real-estate prompt;
- `generate-enhance` для улучшения фото;
- `generate-video` для видео и полного пакета.

Отдельные future workflow для банковской оплаты и выделенных real-estate endpoints не входят в рабочий MVP и не должны активироваться без отдельной настройки.
