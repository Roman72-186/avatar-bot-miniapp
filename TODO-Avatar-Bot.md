...
- [x] Таблицы: `users` (баланс, лимиты), `generations` (история, TTL 24ч)
...
- [x] Баланс звёзд и бесплатные генерации в шапке
...
### Backend (n8n workflows) — 15 воркфлоу
- [x] `user-status` — регистрация/проверка юзера, баланс, лимиты
- [x] `generate` — стилизация (flux/dev/image-to-image)
...
- [x] `generate-remove-bg` — удаление фона (birefnet)
- [x] `generate-enhance` — улучшение качества (clarity-upscaler)
- [x] `generate-text-to-image` — генерация по текстовому описанию
- [x] `create-invoice` — создание инвойса Telegram Stars
- [x] `bot-start-handler` — /start, pre_checkout_query, successful_payment
...
- [ ] **Cron сброс бесплатных лимитов** — ежедневный сброс `free_generations` и `free_reset_date` (n8n Schedule trigger или cron на VPS)
- [ ] **Тестирование на реальных устройствах** — iOS + Android, разные модели
- [ ] **Обработка ошибок** — таймаут fal.ai, плохое фото, сервер недоступен (сейчас показывается сырая ошибка)

### Приоритет 2 — Улучшения для роста
- [ ] **Реферальная система** — deep link `/start ref_USERID`, бонус пригласившему (+3 бесплатные генерации)
...
| Премиум-стили (только за Stars) | Все режимы уже платные (кроме стилизации с лимитом). Искусственное ограничение не нужно |
...
| Покупка без водяного знака (1-2 Stars) | Нет водяного знака -> нет смысла |
